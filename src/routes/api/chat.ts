...
- You do not have live internet access. If the user asks for real-time data, say so plainly and answer with what you reliably know.
...
function toModelMessages(turns: ChatTurn[]): GatewayMessage[] {
  return turns
...
async function streamReply(
  ...
  for (const model of MODEL_FALLBACKS) {
    try {
      ...
      console.log('[chat] model failed:', model, (err as Error)?.message)
    }
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        console.log('[chat] stream failed, using local reply:', (error as Error)?.message)
